...
def analyze_margin_shortfalls(trade_date):
    try:
        ...
        debug_date_query = "SELECT DISTINCT DATE(date) AS trade_date FROM ob"
        try:
            with engine.connect() as connection:
                result = connection.execute(text(debug_date_query))
                available_dates = [row[0].strftime('%Y-%m-%d') for row in result.fetchall()]
            ...
            logger.error(f"Failed to fetch available dates: {str(e)}")
            flash(f"Failed to fetch available dates: {str(e)}", "error")
            return pd.DataFrame(), pd.DataFrame()
        ...
        try:
            with engine.connect() as connection:
                ...
                if not users_data:
                    ...
                    logger.info("Users data is empty")
                    return pd.DataFrame(), pd.DataFrame()
                users_df = pd.DataFrame(users_data, columns=result.keys())
        except Exception as e:
        ...
        try:
            with engine.connect() as connection:
                ...
                orderbook_data = result.fetchall()
                if not orderbook_data:
                    flash(f"No orderbook data found for date {trade_date}.", "error")
                    logger.info(f"Orderbook data is empty for trade_date: {trade_date}")
                    return pd.DataFrame(), pd.DataFrame()
                orderbook_df = pd.DataFrame(orderbook_data, columns=result.keys())
        ...
        if result_df.empty:
            logger.info("No shortfall orders after exchange filter (NFO, BFO)")
            flash("No margin shortfall orders found for NFO or BFO exchanges.", "info")
            return pd.DataFrame(), pd.DataFrame()
    ...
    try:
        ...
        if request.method == 'POST':
            ...
            if not trade_date:
                ...
                logger.warning(f"No trade date provided in POST request, session_id: {session_id}")
            ...
            try:
                ...
                logger.warning(f"Invalid trade_date format: {trade_date}, session_id: {session_id}")
                return render_template('margin_shortfall.html', role=session.get('role'),
                                      result_data=result_data, pivot_data=pivot_data,
                                      trade_date=trade_date)

            if request.form.get('export') == 'xlsx':
                logger.debug(f"Export request received for trade_date: {trade_date}, session_id: {session_id}")
                # Re-run analysis to ensure fresh data
                result_df, pivot_df = analyze_margin_shortfalls(trade_date)
                if result_df.empty or pivot_df.empty:
                    logger.warning(f"Export failed: Empty result_df ({len(result_df)} rows) or pivot_df ({len(pivot_df)} rows) for trade_date: {trade_date}, session_id: {session_id}")
                    flash("No margin shortfall data available to export. Please ensure data exists for the selected date.", "error")
                    return render_template('margin_shortfall.html', role=session.get('role'),
                                          result_data=result_data, pivot_data=pivot_data,
                                          trade_date=trade_date)
                ...
                output.seek(0)
                logger.info(f"Exporting margin shortfall data for trade_date: {trade_date}, session_id: {session_id}")
                return Response(
                    output,
            ...
            # Analyze data
            logger.debug(f"Calling analyze_margin_shortfalls with trade_date: {trade_date}, session_id: {session_id}")
            result_df, pivot_df = analyze_margin_shortfalls(trade_date)
            if result_df.empty or pivot_df.empty:
                logger.info(f"No data returned from analyze_margin_shortfalls for trade_date: {trade_date}, session_id: {session_id}")
                flash("No margin shortfall data found for the selected date.", "error")
                session['margin_trade_date'] = trade_date
                session.modified = True
                return render_template('margin_shortfall.html', role=session.get('role'),
                                      result_data=None, pivot_data=None,
                                      trade_date=trade_date)
            ...
            logger.info(f"Margin shortfall data processed for trade_date: {trade_date}, session_id: {session_id}")
