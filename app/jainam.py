...
if not hasattr(jainam_bp, 'registered'):
    ...
    def save_distribution(row_id):
        ...
        try:
            ...
            for row in partners:
                ...
                connection.execute(text("""
                    INSERT INTO user_partner_data (user_id, alias, allocation, mtm, max_loss, date, broker, algorithm, is_main)
                    VALUES (:user_id, :alias, :allocation, :mtm, :max_loss, :date, :broker, :algorithm, :is_main)
                """), {
                    'user_id': jainam_record['user_id'],
                    'alias': alias,
                    'allocation': alloc,
    ...
    def dashboard():
        ...
        try:
                    ...
                    partners_data = [
                        {
                            ...
                            'algo': partner._mapping['algorithm'] or '',
            ...
            partner_query = text("SELECT * FROM user_partner_data WHERE is_main = :is_main AND broker IN (:b1, :b2, :b3, :b4, :b5)")
            ...
            if start_date and end_date and not date_filter:
                try:
                    ...
                    start_date, end_date = None, None
            partner_query = text(str(partner_query) + " ORDER BY date DESC")
            partner_stats = {}
            for partner in connection.execute(partner_query, partner_params).fetchall():
                partner_dict = partner._mapping
                alias = partner_dict['alias'] or ''
                if alias not in partner_stats:
                    partner_stats[alias] = {'users': []}
                if float(partner_dict['allocation'] or 0) > 0:
                    partner_stats[alias]['users'].append({
                        'row_id': partner_dict['row_id'],
                        'user_id': partner_dict['user_id'],
                        'algo': partner_dict['algorithm'] or '',
                        'mtm': float(partner_dict['mtm'] or 0),
                        'allocation': float(partner_dict['allocation'] or 0),
                        'max_loss': float(partner_dict['max_loss'] or 0),
                        'date': partner_dict['date'].strftime('%Y-%m-%d') if partner_dict['date'] else ''
                    })
    ...
    def export_partner_stats():
        ...
        try:
            ...
            writer.writerow(['Partner', 'Date', 'User ID', 'Algorithm', 'MTM', 'Allocation', 'Max Loss'])
    ...
    def export_user_partner_details():
        ...
        try:
            ...
            output = StringIO()
            writer = csv.writer(output)
            writer.writerow(['User ID', 'Alias', 'Partner', 'Allocation', 'MTM', 'Max Loss', 'Algorithm', 'Broker', 'Date'])
    ...
    def export_csv():
        ...
        try:
            ...
            return Response(
                output,
                mimetype='text/csv',
                headers={'Content-Disposition': 'attachment;filename=jainam.csv'}
            )
        except Exception as e:
